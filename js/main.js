...
function initSmoothScrolling() {
    const lenis = new Lenis({
        duration: 1.8,   // Increased duration for smoother stop
        ...
        touchMultiplier: 1.5,
        infinite: false,
        wheelMultiplier: 0.8, // Reduced wheel multiplier for smoother scrolling
    ...
    lenis.on('scroll', (e) => {
        // Optional: Do something on scroll events.
    });
...
// Services Experience Functionality
document.addEventListener('DOMContentLoaded', function() {
    ...
    function switchPanel(tabId) {
        ...
        // Add active classes
        document.querySelector(`.exp-tab[data-tab="${tabId}"]`).classList.add('active');
        document.querySelector(`.exp-panel[data-panel="${tabId}"]`).classList.add('active');
    }
    ...
    const expImages = document.querySelectorAll('.exp-image-main img, .exp-image-side img');
    ...
    window.addEventListener('scroll', () => {
        expImages.forEach(img => {
            ...
            if (parentRect.top < window.innerHeight && parentRect.bottom > 0) {
                ...
                const scrollPercent = (window.innerHeight - parentTop) / (window.innerHeight + parentRect.height);
                const moveAmount = Math.min(50, Math.max(-50, scrollPercent * 100 - 50));
